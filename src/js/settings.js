document.addEventListener('DOMContentLoaded', () => {
    ...
    // NEW: Separate scan method controls
    const allowBypassIgnoredToggle = document.getElementById('allowBypassIgnoredToggle');
    const separateScanMethodsToggle = document.getElementById('separateScanMethodsToggle');
    const unifiedScanMethodItem = document.getElementById('unifiedScanMethodItem');
    const basicScanMethodItem = document.getElementById('basicScanMethodItem');
    const fullScanMethodItem = document.getElementById('fullScanMethodItem');
    const basicScanMethodDropdown = document.getElementById('basicScanMethodDropdown');
    const fullScanMethodDropdown = document.getElementById('fullScanMethodDropdown');
    ...
    if (!hideWebsiteToggle || !autoOpenToggle || !debugModeDropdown || !bypassModeDropdown || 
        ...
        !uiDebugModeToggle || !networkDebugModeToggle || !allowBypassIgnoredToggle || 
        !separateScanMethodsToggle || !basicScanMethodDropdown || !fullScanMethodDropdown) {
    ...
    async function loadSettings() {
        try {
            ...
            allowBypassIgnoredToggle.checked = settings.allow_bypass_ignored || false;  // NEW
            ...
            // NEW: Separate scan methods
            separateScanMethodsToggle.checked = settings.separate_scan_methods || false;
            basicScanMethodDropdown.value = settings.basic_scan_method || 'divide_and_conquer';
    ...
    function getUpdatedSettings() {
        return {
            ...
            allow_bypass_ignored: allowBypassIgnoredToggle.checked,  // NEW
            ...
            // NEW: Separate scan methods
            separate_scan_methods: separateScanMethodsToggle.checked,
            basic_scan_method: basicScanMethodDropdown.value,
            full_scan_method: fullScanMethodDropdown.value,
    ...
    function initializeTooltips() {
        ...
        // NEW: Tooltip for Allow Bypass on Ignored Adapters
        createInfoTooltip('allowBypassIgnoredInfo', `
            <h3>Allow Bypass on Ignored Adapters</h3>
            <div class="tooltip-item">
                <p>When enabled, virtual and ignored network adapters (like VirtualBox, VMware, Hyper-V) will appear in the bypass list and can be used for MAC address changes.</p>
            </div>
            <div class="tooltip-warning">
                Warning: Changing MAC addresses on virtual adapters may cause connectivity issues with VMs or virtual networks.
            </div>
        `);

        // NEW: Tooltip for Separate Scan Methods
        createInfoTooltip('separateScanMethodsInfo', `
            <h3>Use Different Scan Methods</h3>
            <div class="tooltip-item">
                <p>When enabled, you can choose different scanning methods for basic and full scans separately.</p>
            </div>
            <div class="tooltip-item">
                <p><strong>Use Case:</strong> Use a fast method like "Smart" for basic scans and a more thorough method like "Divide and Conquer" for full scans.</p>
            </div>
        `);

        // NEW: Tooltip for Basic Scan Method
        createInfoTooltip('basicScanMethodInfo', `
            <h3>Basic Scan Method</h3>
            <div class="tooltip-item">
                <p>The scanning method used for quick basic scans (IP and MAC only).</p>
            </div>
            <div class="tooltip-item">
                <p><strong>Recommendation:</strong> Use "Smart" for fastest results on basic scans.</p>
            </div>
        `);

        // NEW: Tooltip for Full Scan Method
        createInfoTooltip('fullScanMethodInfo', `
            <h3>Full Scan Method</h3>
            <div class="tooltip-item">
                <p>The scanning method used for thorough full scans (with hostname, vendor, and OS detection).</p>
            </div>
            <div class="tooltip-item">
                <p><strong>Recommendation:</strong> Use "Smart" or "Divide and Conquer" for balanced speed and accuracy.</p>
            </div>
        `);
    }

    function toggleSeparateScanMethods(enabled) {
        if (enabled) {
            // Hide unified dropdown, show separate dropdowns
            unifiedScanMethodItem.style.display = 'none';
            basicScanMethodItem.style.display = '';
            fullScanMethodItem.style.display = '';
        } else {
            // Show unified dropdown, hide separate dropdowns
            unifiedScanMethodItem.style.display = '';
            basicScanMethodItem.style.display = 'none';
            fullScanMethodItem.style.display = 'none';
        }
    }
    ...
    const allSettingsControls = [
        ...
        allowBypassIgnoredToggle, separateScanMethodsToggle,  // NEW
        basicScanMethodDropdown, fullScanMethodDropdown  // NEW
    ];
    ...
    if (separateScanMethodsToggle) {
        separateScanMethodsToggle.addEventListener('change', () => {
            toggleSeparateScanMethods(separateScanMethodsToggle.checked);
        });
    }
