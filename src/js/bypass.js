...
async function refreshAdapters(forcedBypassMode = null) {
    try {
        ...
        list.innerHTML = adapters.map(adapter => {
            ...
            const bypassAllowed = adapter.bypass_allowed !== false;  // NEW: Check if bypass is allowed
                ...
                <div class="adapter-card ${adapter.ignored ? 'ignored' : ''}">
                    ...
                    <div class="adapter-card-body">
                        ${isRegistryMode ? `
                            <div class="control-group">
                                ...
                                <select id="bypassMode-${adapter.transport}" class="bypass-mode-dropdown dropdown" data-transport="${adapter.transport}" ${!bypassAllowed ? 'disabled' : ''}>
                            ...
                            <div class="control-group manual-mac-input-container" id="manualMacContainer-${adapter.transport}" style="display: none;">
                                ...
                                <input type="text" id="manualMac-${adapter.transport}" class="hotspot-input manual-mac-input" placeholder="00:1A:2B:3C:4D:5E" ${!bypassAllowed ? 'disabled' : ''}>
                            </div>
                        ` : ''}
                    </div>
                    <div class="adapter-card-footer">
                        <button class="btn btn-primary btn-bypass" 
                                ...
                                ${!bypassAllowed ? 'disabled' : ''}>
                            ${!bypassAllowed ? 'Bypass Disabled' : `Bypass (${bypassMode})`}
                        </button>
                    </div>
                </div>
            `;
