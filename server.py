...
TUTORIAL_FILE = os.path.join(APPDATA_LOCATION, "tutorial_completed.json")
...
DEFAULT_SETTINGS = {
    ...
    "allow_bypass_ignored": False,
    "separate_scan_methods": False,  
    "basic_scan_method": "divide_and_conquer", 
    "full_scan_method": "divide_and_conquer"  
}
...
def get_adapters():
    try:
        ...
        allow_bypass_ignored = settings.get("allow_bypass_ignored", False)
        ...
        for idx, name in enumerate(transport_names_list[:5]):
            ...
            if is_ignored and not show_ignored and not allow_bypass_ignored:
                continue
            ...
            adapters.append({
                ...
                "ignored": is_ignored,
                "bypass_allowed": not is_ignored or allow_bypass_ignored
            })
    ...
    try:
        if os.path.exists(TUTORIAL_FILE):
            with open(TUTORIAL_FILE, 'r') as f:
                data = json.load(f)
                return jsonify({"completed": data.get("completed", False)})
...
def complete_tutorial():
    try:
        ...
        return jsonify({"message": "Tutorial marked as completed"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def basic_scan():
    try:
        ...
        # NEW: Check if separate scan methods are enabled
        use_separate = settings.get("separate_scan_methods", False)
        if use_separate:
            scanning_method = settings.get("basic_scan_method", "divide_and_conquer")
        else:
...
def full_scan():
    try:
        ...
        use_separate = settings.get("separate_scan_methods", False)
        if use_separate:
            scanning_method = settings.get("full_scan_method", "divide_and_conquer")
        else:
            scanning_method = settings.get("scanning_method", "divide_and_conquer")
...
def settings_route():
    ...
    settings_data['is_electron'] = is_running_in_electron()
...
def exit_server():
    ...
    # Start shutdown in a separate thread so response can be sent first
...
def print_full_debug_startup_info():
    try:
        ...
        print("\033[94m[DEBUG] System: %s %s (%s)\033[0m" % (platform.system(), platform.release(), platform.version()))
        print("\033[94m[DEBUG] Python: %s\033[0m" % sys.version)
        print("\033[94m[DEBUG] CPU: %s (%d threads @ %.2f MHz)\033[0m" % (cpu_info.get('brand_raw', 'N/A'), psutil.cpu_count(logical=True), psutil.cpu_freq().current))
        print("\033[94m[DEBUG] RAM: %.2f GB\033[0m" % memory_total_gb)
        print("\033[94m[DEBUG] Storage Devices:\033[0m")
        for d in storage_devices:
            print(f"  \033[94m[DEBUG] - {d['device']} ({d['mount']}) - {d['total_gb']} GB\033[0m")
        ...
        app.logger.warning(f"\033[93m[WARN] Failed to produce full debug snapshot: {e}\033[0m")
...
if __name__ == '__main__':
    ...
    # Log core component status when packaged
    if getattr(sys, 'frozen', False):
        auto_bypass_path = os.path.join(sys._MEIPASS, 'auto_bypass_service.exe')
        app.logger.info(f"Core aspect auto_bypass_service.exe loaded: {'Y' if os.path.exists(auto_bypass_path) else 'N'}")
    ...
    try:
        ...
        print("\033[93m[INFO] Keyboard interrupt received.\033[0m")
        graceful_shutdown(signal.SIGINT, None)
    except Exception as e:
